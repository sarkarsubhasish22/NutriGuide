...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        if (!canScroll) {
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, ShapeTypes.DOUBLE_WAVE);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(ShapeTypes.DOUBLE_WAVE)) {
                return false;
            }
            return true;
            ...
            if (keyCode != 19) {
                if (keyCode != 20) {
                    ...
                    return arrowScroll(ShapeTypes.DOUBLE_WAVE);
                } else {
                    return fullScroll(ShapeTypes.DOUBLE_WAVE);
                }
            } else if (!event.isAltPressed()) {
    ...
    public boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = ShapeTypes.DOUBLE_WAVE;
        } else if (direction == 1) {
            direction = 33;
        }
