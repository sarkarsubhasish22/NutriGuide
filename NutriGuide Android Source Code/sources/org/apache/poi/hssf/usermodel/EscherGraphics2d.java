...
public final class EscherGraphics2d extends Graphics2D {
    ...
    public void draw(Shape shape) {
        if (shape instanceof Line2D) {
            ...
            if (basicStroke != null && (basicStroke instanceof BasicStroke)) {
                width = ((int) basicStroke.getLineWidth()) * HSSFShape.LINEWIDTH_ONE_PT;
            }
            drawLine((int) shape2d.getX1(), (int) shape2d.getY1(), (int) shape2d.getX2(), (int) shape2d.getY2(), width);
        } else if (this.logger.check(POILogger.WARN)) {
            this.logger.log(POILogger.WARN, (Object) "draw not fully supported");
        }
    }

    public void drawArc(int x, int y, int width, int height, int startAngle, int arcAngle) {
        draw(new Arc2D.Float((float) x, (float) y, (float) width, (float) height, (float) startAngle, (float) arcAngle, 0));
    }
    ...
    public boolean drawImage(Image image, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2, Color bgColor, ImageObserver imageobserver) {
        ...
        this.logger.log(POILogger.WARN, (Object) "drawImage() not supported");
        return true;
    }
    ...
    public boolean drawImage(Image image, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2, ImageObserver imageobserver) {
        ...
        return drawImage(image, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2, (Color) null, imageobserver);
    }
    ...
    public boolean drawImage(Image img, int x, int y, int width, int height, ImageObserver observer) {
        return drawImage(img, x, y, width, height, (Color) null, observer);
    }
    ...
    public boolean drawImage(Image image, AffineTransform affinetransform, ImageObserver imageobserver) {
        ...
        setTrans((AffineTransform) getTrans().clone());
        return true;
    }

    public void drawImage(BufferedImage bufferedimage, BufferedImageOp op, int x, int y) {
        drawImage(op.filter(bufferedimage, (BufferedImage) null), new AffineTransform(1.0f, 0.0f, 0.0f, 1.0f, (float) x, (float) y), (ImageObserver) null);
    }
    ...
    public void drawRenderedImage(RenderedImage renderedimage, AffineTransform affinetransform) {
        ...
        drawImage(bufferedimage, affinetransform, (ImageObserver) null);
    }
    ...
    public void fill(Shape shape) {
        if (this.logger.check(POILogger.WARN)) {
            this.logger.log(POILogger.WARN, (Object) "fill(Shape) not supported");
        }
    }

    public void fillArc(int i, int j, int k, int l, int i1, int j1) {
        fill(new Arc2D.Float((float) i, (float) j, (float) k, (float) l, (float) i1, (float) j1, 2));
    }
